...
import {
    ...
    MessageSquare, Download, Dices, Maximize2, Eraser, FlaskConical
} from 'lucide-react';
...
import Lab from './Lab.js'; // Impor komponen Lab
...
export default function AIImageGenerator() {
  ...
  if (!isMounted) return <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900"><Spinner/></div>;
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 bg-[var(--bg-color)] text-[var(--text-color)]`}>
      ...
      {isAdminModalOpen && <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4"><div className="p-8 rounded-2xl w-full max-w-md" style={{ background: 'var(--bg-color)', boxShadow: 'var(--shadow-outset)' }}><div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold">Panel Admin</h2><NeumorphicButton onClick={() => setIsAdminModalOpen(false)} className="!p-2"><X size={20} /></NeumorphicButton></div><p className="mb-4 text-sm"> Masukkan password admin untuk mengakses fitur. atau hubungi Admin ruangriung di halaman <a style={{color:"#3b82f6"}} href="https://web.facebook.com/groups/1182261482811767/" target="_blank" rel="noopener noreferrer">Facebook RuangRiung</a>.
          </p><div className="relative w-full mb-4"><label htmlFor="admin-password-input" className="sr-only">Password Admin</label><input id="admin-password-input" type={showAdminPassword ? "text" : "password"} value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Password Admin" className="w-full p-3 rounded-lg neumorphic-input pr-12"/><button type="button" aria-label={showAdminPassword ? "Sembunyikan password" : "Tampilkan password"} onClick={() => setShowAdminPassword(!showAdminPassword)} className="absolute inset-y-0 right-0 pr-3 flex items-center">{showAdminPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button></div><div className="space-y-4"><NeumorphicButton onClick={handleAdminReset} className="font-bold w-full"><RefreshCw size={16}/> Reset Koin</NeumorphicButton></div></div></div>}
      ...
      {isApiModalOpen && <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4"><div className="p-8 rounded-2xl w-full max-w-md" style={{ background: 'var(--bg-color)', boxShadow: 'var(--shadow-outset)' }}><div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold">API Key untuk {modelRequiringKey?.toUpperCase()}</h2><NeumorphicButton onClick={() => setIsApiModalOpen(false)} className="!p-2"><X size={20} /></NeumorphicButton></div><p className="mb-4 text-sm">Model ini memerlukan API key yang valid.</p><div className="relative w-full mb-4"><label htmlFor="api-key-input" className="sr-only">API Key</label><input id="api-key-input" type={showApiKey ? "text" : "password"} value={tempApiKey} onChange={(e) => setTempApiKey(e.target.value)} placeholder="Masukkan API Key Anda" className="w-full p-3 rounded-lg neumorphic-input pr-12"/><button type="button" aria-label={showApiKey ? "Sembunyikan API Key" : "Tampilkan API Key"} onClick={() => setShowApiKey(!showApiKey)} className="absolute inset-y-0 right-0 pr-3 flex items-center">{showApiKey ? <EyeOff size={20} /> : <Eye size={20} />}</button></div><div className="flex justify-end gap-4"><NeumorphicButton onClick={() => setIsApiModalOpen(false)}>Batal</NeumorphicButton><NeumorphicButton onClick={handleApiKeySubmit} className="font-bold">Simpan</NeumorphicButton></div></div></div>}
                            ...
                            <div className='space-y-4 animate-fade-in'>
                              ...
                              <NeumorphicButton onClick={() => setIsCreatorOpen(!isCreatorOpen)} className="w-full text-sm !p-3">
                                ...
                                <span className="absolute right-3 top-1/2 -translate-y-1/2">
                                  {isCreatorOpen ? <ChevronUp size={16}/> : <ChevronDown size={16}/>}
                                </span>
                              </NeumorphicButton>
                              {isCreatorOpen && 
                                  <div className="p-4 rounded-lg space-y-4 animate-fade-in" style={{boxShadow: 'var(--shadow-inset)'}}>
                                      <div>
                                          ...
                                          <input id="prompt-creator-subject" name="subject" value={promptCreator.subject} onChange={(e) => handlePromptCreatorChange(e, 'image')} placeholder="cth: seekor kucing astronot" className="w-full p-2 rounded-lg neumorphic-input text-sm" />
                                      </div>
                                      <div>
                                          <label className="text-xs font-semibold block mb-1">Detail Tambahan</label>
                                          <textarea id="details-textarea" name="details" value={promptCreator.details} onChange={(e) => handlePromptCreatorChange(e, 'image')} placeholder="cth: hyperrealistic, 4k" className="w-full p-2 rounded-lg neumorphic-input text-sm h-20 resize-none" />
                                      </div>
                      ...
                      {activeTab === 'lab' && !loading && (
                          <div className="w-full flex justify-center">
                            <div className="rounded-2xl p-8 text-center text-gray-400 bg-[var(--bg-color)] border border-gray-200 dark:border-gray-800 neumorphic-card shadow-lg max-w-3xl w-full">
                                {isLabAuthenticated
                                ? 'Hasil eksperimen Lab akan muncul di sini.'
                                : 'Masukkan kata sandi untuk melihat konten Lab.'}
                            </div>
                          </div>
                      )}
                                ...
                                <div className="max-h-96 overflow-y-auto space-y-2 pr-2">
                                  {generationHistory.length === 0 ? <p className="text-sm opacity-60">Kosong</p> : generationHistory.map((h) => (
                                      <div key={h.date} className="flex items-center gap-2 p-2 rounded-lg transition-all hover:bg-[var(--shadow-dark)]/20" style={{boxShadow:'var(--shadow-inset)'}}>
                                        <img src={h.url} alt={h.prompt.substring(0,30)} className="w-16 h-16 rounded-md object-cover cursor-pointer flex-shrink-0" onClick={() => { setSelectedHistoryImage(h); setGeneratedImages([]); window.scrollTo({ top: 0, behavior: 'smooth' }); }}/>
                                        <p className="text-xs line-clamp-3 flex-grow cursor-pointer" onClick={() => { setSelectedHistoryImage(h); setGeneratedImages([]); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>{h.prompt}</p>
