                                    ...
                                    <div>
                                        <div className="flex items-center justify-between mb-2">
                                            ...
                                            <button
                                              ...
                                              aria-label={useCustomSize ? 'Gunakan preset ukuran gambar' : 'Gunakan ukuran kustom'}
                                              type="button"
                                            >
                                              {useCustomSize ? 'Preset' : 'Kustom'}
                                            </button>
                                        </div>
                                        {!useCustomSize ? <select id="size-preset-select" value={sizePreset} onChange={(e) => setSizePreset(e.target.value)} className="w-full p-3 rounded-lg neumorphic-input bg-[var(--bg-color)]"><option value="1024x1024">1024x1024</option><option value="1024x1792">1024x1792</option><option value="1792x1024">1792x1024</option></select> : <div className="space-y-3 p-3 rounded-lg" style={{boxShadow: 'var(--shadow-inset)'}}><div><label htmlFor="custom-width-input" className="text-sm">Width: {customWidth}px</label><input id="custom-width-input" type="range" min="256" max="2048" step="64" value={customWidth} onChange={(e) => setCustomWidth(Number(e.target.value))} className="w-full" aria-valuenow={customWidth} aria-valuemin={256} aria-valuemax={2048} aria-label="Lebar gambar" /></div><div><label htmlFor="custom-height-input" className="text-sm">Height: {customHeight}px</label><input id="custom-height-input" type="range" min="256" max="2048" step="64" value={customHeight} onChange={(e) => setCustomHeight(Number(e.target.value))} className="w-full" aria-valuenow={customHeight} aria-valuemin={256} aria-valuemax={2048} aria-label="Tinggi gambar" /></div></div>}
                                    </div>
