    ...
    return (
        <div className="border-t border-gray-500/20 pt-2 mt-4">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center py-2 text-left">
                <h4 className="font-semibold flex items-center gap-2"><span className="text-lg">{icon}</span> {title}</h4>
...
export default function AIImageGenerator() {
  ...
  const colorGradingOptions = ["Neutral", "Teal & Orange", "Cool Tone", "Warm Tone", "High Contrast", "Monochrome", "Vintage"];
                    ...
                    <div className="p-6 rounded-2xl h-fit space-y-4 neumorphic-card">
                        ...
                        {activeTab === 'image' && (
                            <div className='space-y-4'>
                                <label className="font-semibold block text-xl">Prompt Gambar</label>
                                ...
                                <div className="flex flex-wrap gap-2"><NeumorphicButton onClick={handleEnhancePrompt} loading={isEnhancing} loadingText="Memproses..." className="flex-1 text-sm"><Wand2 size={16}/>Sempurnakan</NeumorphicButton><NeumorphicButton onClick={() => { if (!prompt.trim()) { showToast('Prompt kosong tidak bisa disimpan', 'error'); return; } if (savedPrompts.some(p => p.prompt === prompt.trim())) { showToast('Prompt ini sudah ada di favorit.', 'info'); return; } setSavedPrompts(p => [{prompt: prompt.trim(), date: new Date().toISOString()}, ...p]); showToast('Prompt disimpan!', 'success'); }} className="flex-1 text-sm"><Bookmark size={16}/> Simpan</NeumorphicButton></div>
